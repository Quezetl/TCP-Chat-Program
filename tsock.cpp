...
int tsock::display()
{
	do
	{
		...
		else if(msg == "4R52L" && EC == 0) {
			for (int i = 0; i < ClientsInfo.size(); i++) {
				if (cPort== ClientsInfo[i][1] && cIP == ClientsInfo[i][0]) {
					// Error check
					...
					// Deletes the selects connection from ClientSock and ClientsInfo vectors
					closesocket(ClientSock.at(i));
					ClientSock.erase(ClientSock.begin() + i);
					ClientsInfo.erase(ClientsInfo.begin() + i);

					cout << "Connection id: " << i+1 << " has been terminated" << endl;
					break;
				}
			}
		}
...
{
	for (int i = 0; i < ClientsInfo.size(); i++) {
		if (ClientsInfo[i].at(0) == dest && ClientsInfo[i].at(1) == prt ){
			cout << "Connection already made\n";
			return 0;
		}
	}

	if ((dest == IPADD && prt == Serverport) ||(dest == "127.0.0.1" && prt == Serverport) ){
		cout << "No self-Connections allowed.\n";
		return 0;
	}
	for (int i = 0; i < ClientsInfo.size(); i++)
	{
		if (ClientsInfo[i][1] == prt) {
	...
	send(ClientSock.back(), (IPADD + ' ' + Serverport + ' ').c_str(), 100, 0);
	return 0;
}
...
void tsock::List()
{
	cout << "Id:" << setw(16) << "IP Adress"
		<< setw(16) << "Port No." << endl;
...
int tsock::Terminate(int id)
{
	...
	//Special Terminate msg
	EC = send(ClientSock.at(id - 1), (const char*)(IPADD + ' ' + Serverport + ' ' + "4R52L").c_str(), 100, 0);
	...
	cout << "Connection id: " << id << " terminated" << endl;
	return 0;
}
